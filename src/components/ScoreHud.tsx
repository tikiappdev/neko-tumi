...
export interface ScoreHudProps { score: number; floors: number; bestScore: number; combo?: number; soundEnabled: boolean; onToggleSound: () => void; onExit: () => void }
export function ScoreHud({ score, floors, bestScore, combo = 0, soundEnabled, onToggleSound, onExit }: ScoreHudProps) {
  return <header className="score-hud">
    ...
    <div className={`combo-pill ${combo > 1 ? 'combo-pill--show' : ''}`} aria-live="polite">⭐ ぴったり × {combo}</div>
  </header>;
}
